...
Some mathematicians or programmers might turn their nose up at referring to our triangle points as vectors. They'll say a vector doesn't encode position, just a length (magnitude) and angle. I would say that in the context we're using vectors, the distinction doesn't really matter. In our video, triangle point `c` at `(2, 4)` in the top left is from the origin, moving up four and right two units. It can also be a displacement from `a` represented as the vector `v = (1, 3)`, moving up three and right 1 from point `a`. As I mentioned earlier, we could have treated `a`, `b` and `c` as vectors and chosen two of them to plug into our cross product formula, but we would have gotten a triangle with the origin at `(0, 0)` as one of its points. You can think of the triangle in terms of the edges that enclose it. Vectors `u` and `v` enclose the triangle `abc` while vectors `b` and `c` for example enclose triangle `Obc` if we label the origin as `O`. The reason we calculate `u` and `v` is so that all three of the triangle positions are encoded into two vectors.
