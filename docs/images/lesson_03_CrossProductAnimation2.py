...
class CrossProductAnimation(Scene):
    def construct(self):
        ...
        def narration(text, prev=None, rt=0.8):
        ...
        # (0,0) label sits in quadrant 2 — up and left of the origin dot
        lbl_o = T("(0,0)", size=14, color=GREY_B).move_to(PO + UP * 0.28 + LEFT * 0.40)
        self.play(FadeIn(dot_o), FadeIn(lbl_o), GrowArrow(arr_a))
        ...
        # Second beat under 4a — split from 4b so the idea lands before the demo
        narr = narration("We can choose any vector to be the origin.", narr)
        self.wait(5)

        # ── FRAME 4b: Demonstrate what goes wrong using origin vectors ────────
        narr = narration(
            "If we use two vectors from the screen origin (0, 0)\nlike vectors a and c, we get a different triangle than on ABC.",
            narr)

        # Dim b and c before showing the wrong triangle
        self.play(arr_b.animate.set_opacity(0.25),
                  lbl_b.animate.set_opacity(0.25))

        # Shade the wrong triangle: origin, A, C
        wrong_tri = Polygon(PO, PA, PC,
                            fill_color=C_PARA_NEG, fill_opacity=0.28,
                            stroke_color=C_PARA_NEG, stroke_width=1.5)
        self.play(DrawBorderThenFill(wrong_tri), run_time=3.0)
        self.wait(6)

        # Narration and wrong_tri fade play simultaneously. Both arr_b/lbl_b and
        # arr_c/lbl_c are restored to full opacity so all three vectors are visible.
        _narr_instead = T(
            "Instead, we need two new edge vectors\nfrom a shared origin on the triangle.",
            size=18, color=C_NARR).move_to(NARR_Y)
        self.play(
            FadeIn(_narr_instead, run_time=0.8),
            FadeOut(narr, run_time=0.8),
            FadeOut(wrong_tri),
            arr_b.animate.set_opacity(1.0),
            lbl_b.animate.set_opacity(1.0),
            arr_c.animate.set_opacity(1.0),
            lbl_c.animate.set_opacity(1.0),
        )
        narr = _narr_instead
        self.wait(5)

        # ── FRAME 4b cont: Pick a as the shared origin ────────────────────────
        narr = narration(
            "We can choose any vector on the triangle as the origin — let's pick vector a.",
            narr)
        ...
        # ── FRAME 4c ──────────────────────────────────────────────────────────
        # Fade b and c (and their labels) out on this narration line so the
        # viewer sees only a and its arrow while the new vectors are named
        narr = narration(
            "From a, we form two new vectors — one toward b, one toward c.",
            narr)
        self.play(arr_b.animate.set_opacity(0.25),
                  lbl_b.animate.set_opacity(0.25),
                  arr_c.animate.set_opacity(0.25),
                  lbl_c.animate.set_opacity(0.25),
                  FadeOut(lbl_o))
        self.wait(2)
        ...
        self.play(DrawBorderThenFill(para), FadeOut(ring))
        ...
        f_gen0    = T("u x v = (u.y*v.z - u.z*v.y,", size=17, color=C_FORM)
        f_gen1    = T("u.z*v.x - u.x*v.z,",           size=17, color=C_FORM)
        f_gen2    = T("u.x*v.y - u.y*v.x)",            size=17, color=C_FORM)
        place_col([f_gen_hdr, f_gen0, f_gen1, f_gen2], v_bot, gap=0.20)
        # Shift f_gen1/2 right so their left edge aligns with the "u" of "u.y" in f_gen0.
        # Measure the prefix "u x v = (" as a temporary Text to get exact pixel width.
        _gen_prefix_w = T("u x v = (u", size=17, color=C_FORM).width
        f_gen1.shift(RIGHT * _gen_prefix_w)
        f_gen2.shift(RIGHT * _gen_prefix_w)
        ...
        f_sub0    = T("u x v = (u.y*0 - 0*v.y,", size=17, color=C_FORM)
        f_sub1    = T("0*v.x - u.x*0,",           size=17, color=C_FORM)
        f_sub2    = T("u.x*v.y - u.y*v.x)",        size=17, color=C_FORM)
        place_col([f_sub_hdr, f_sub0, f_sub1, f_sub2], gen_bot, gap=0.20)
        _sub_prefix_w = T("u x v = (u", size=17, color=C_FORM).width
        f_sub1.shift(RIGHT * _sub_prefix_w)
        f_sub2.shift(RIGHT * _sub_prefix_w)
